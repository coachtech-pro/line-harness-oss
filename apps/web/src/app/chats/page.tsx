            ...
            try {
                ...
                const collectText = (obj: Record<string, unknown>) => {
                    ...
                    for (const key of ["header", "body", "footer"]) {
                        if (obj[key])
                            collectText(obj[key] as Record<string, unknown>);
                    }
                    if (Array.isArray(obj.contents)) {
                        for (const c of obj.contents)
                            collectText(c as Record<string, unknown>);
                    }
                };
                collectText(parsed);
...
export default function ChatsPage() {
    ...
    useEffect(() => {
        try {
            ...
            if (rawEnabled !== null)
                setShowLoadingIndicator(rawEnabled === "1");
            if (rawSeconds) {
    ...
    useEffect(() => {
        try {
            localStorage.setItem(
                SHOW_LOADING_PREF_KEY,
                showLoadingIndicator ? "1" : "0",
            );
            localStorage.setItem(
                LOADING_SECONDS_PREF_KEY,
                String(loadingSeconds),
            );
        } catch {
            // localStorage unavailable
        }
    ...
    const loadAllFriends = useCallback(async () => {
        try {
            ...
            if (friendRes.success) {
                setAllFriends(
                    (friendRes.data as unknown as { items: FriendItem[] })
                        .items,
                );
            }
        } catch {
    ...
    useEffect(() => {
        try {
            ...
            if (saved === "enter" || saved === "shift-enter")
                setSendMode(saved);
        } catch {
            /* localStorage unavailable */
        }
    ...
    useEffect(() => {
        try {
            localStorage.setItem("chat.sendMode", sendMode);
        } catch {
            /* ignore */
        }
    }, [sendMode]);
    ...
    const triggerLoadingAnimation = useCallback(
        async (chatId: string) => {
        ...
        try {
            ...
            setChats((prev) => {
                ...
                const filtered =
                    currentFilter === "all"
                        ? updated
                        : updated.filter((c) => c.status === currentFilter);
                            ...
                            <div className="px-4 py-4 border-b border-gray-200 flex items-center justify-between gap-2">
                                <div className="flex items-center gap-2 min-w-0">
                                    ...
                                    <div className="min-w-0">
                                        ...
                                        >
                                            {
                                                statusConfig[chatDetail.status]
                                                    .label
                                            }
                                        </span>
                                    </div>
